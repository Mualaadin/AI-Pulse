...
class ChatbotAssistant:
    ...
    def train_model(self, batch_size=8, lr=0.001, epochs=100):
        ...
        print(f"Training model with {len(self.intents)} intents and {self.X.shape[1]} vocabulary words...")
...
if __name__ == '__main__':
   ...
   # LOAD EXISTING MODEL (comment the above and uncomment below to use existing model)
    print("=== LOADING EXISTING MODEL ===")
    assistant = ChatbotAssistant(INTENTS_FILE, {'stocks': get_stocks})
    assistant.parse_intents()  # Still need this for responses
